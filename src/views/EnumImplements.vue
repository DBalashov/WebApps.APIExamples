    ...
    <div class="column is-4">
      <b-tabs :animated="false">
        <b-tab-item label="JQuery">
          <div class="example">
<pre>
$.post("{{getSettings().Urls.Service}}/EnumImplements",
{
    session: "{{shorten(getSettings().Token)}}",
    schemaID: "{{getSettings().Organization.UID}}"
}).done((r: IEnumImplementsResult) => {
    // r.Items, r.Groups
})
</pre>
          </div>
        ...
        <b-tab-item label="ServiceConnector">
          <div class="example">
<pre>
this.connector.EnumImplements().done((r: IEnumImplementsResult) => {
    // r.Items,
    // r.Groups
})
</pre>
    ...
    <div class="column is-8">
      <b-tabs :animated="false">
        <b-tab-item label="Result as grid">
          <div>
            <table class="items" v-if="store.state.Implements.Groups.length">
              <tr>
              ...
              <tr v-for="o in store.state.Implements.Groups">
                <td>{{o.ID}}</td>
          ...
          <div>
            <table class="items" v-if="store.state.Implements.Items.length">
              <tr>
              ...
              <tr v-for="o in store.state.Implements.Items">
                <td>{{o.ID}}</td>
                <td>{{o.ParentID || 'null'}}</td>
                <td>{{o.Name}}</td>
                <td>{{o.Image}}</td>
...
<script lang="ts">
  ...
  @Component
  export default class EnumImplementsView extends EnumBase {
    ...
    execute() {
      this.updateImplements().then(r => this.Content = JSON.stringify(r, null, "  "));
    }
  }
</script>
